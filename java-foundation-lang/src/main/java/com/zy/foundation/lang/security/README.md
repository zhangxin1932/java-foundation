...
具体包括SM1、SM2、SM3、SM4等，其中：
>> SM1 为对称加密算法，加密强度为128位，采用硬件实现；
>> SM2 为非对称加密算法, 是国家密码管理局公布的公钥算法，其加密强度为256位；
>> SM3 为密码杂凑算法，杂凑值长度为32字节；
>> SM4 为对称加密算法，可使用软件实现，加密强度为128位，主要用于无线局域网产品。
```
